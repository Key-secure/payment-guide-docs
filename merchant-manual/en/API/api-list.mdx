...
| Account | POST | /open-api/v1/reserve/fund/transaction/list | Reserve fund balance details list |
...
Sandbox integration APIs: simulate card transactions and push Webhooks to your callback URL. **Do not use** the `Api-Key` / `Access-Token` headers. See [Transaction Simulation Overview](/en/API/api-list/mock/overview).

Path prefix:
- Sandbox: `https://sandbox-openplatform.keysecure.io/open-api/v1/simulate`

| Module | Method | Path suffix | Description |
| ---- | ---- | ---- | ---- |
...
| Simulation | POST | /external_api/simulate_tevau_kyc | Simulate KYC review (Hong Kong card BINs only) |
...
**Balance Flow:**
1. Query balance details → `POST /reserve/fund/transaction/list`
...
- `/reserve/fund/transaction/list` — Balance details
