...
All interface paths use `/open-api` prefix.

Except for **Get Token**, all other interfaces require headers: `Api-Key`, `Timestamp`, `Access-Token`.
...
| Card | GET | /open-api/v1/card/{card_no}/private/info | Card privacy JSON (`data` is AES encrypted string) |
...
Testing interfaces. Query database and simulate Webhook callbacks to business system. **Do not use** standard headers `Api-Key` / `Access-Token`.
...
- Requires merchant to enable PCI
