...
2. **Query Reserve Fund Balance Details** — Query deposits, withdrawals, consumption, card issuance, and shipping by time range
3. **Query Account Balance** — Get cardholder account info to confirm available assets
4. **Execute Fund Transfer** — Perform fund transfer when cardholder funds change (top-up or withdrawal)
5. **Query Transfer Flow** — Check transfer history for reconciliation
...
| Query Reserve Fund Balance Details | POST | `/open-api/v1/reserve/fund/transaction/list` | Query reserve fund account balance details by time range, up to 31 days |
