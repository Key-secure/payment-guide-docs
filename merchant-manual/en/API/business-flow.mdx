...
Complete payment card service integration consists of four stages, from user authentication to consumption reconciliation. The following details each stage and corresponding interfaces:
...
2. **Initiate Cardholder Authentication** — Call interface to create cardholder and initiate authentication
...
| **Cardholder Authentication Application** | POST | `/open-api/v1/cardholder/apply` | Create cardholder and initiate authentication; after authentication failure, can update authentication info and re-initiate |
| Get Cardholder List | POST | `/open-api/v1/cardholder/list` | Get current cardholder list |
| Get Cardholder Details | GET | `/open-api/v1/cardholder/{cardholder_no}/info` | Get cardholder's card list and assets; can verify authentication status |
...
**Scenario**: After cardholder authentication passes, user selects card type (virtual/physical) and applies for card.
...
| --- | --- | --- | --- | --- |
| Get Card Package List | POST | `/open-api/v1/card/package/list` | Credit | Login merchant backend to create card packages; charge based on package when applying |
| Get Supported Card BIN List | POST | `/open-api/v1/card/support/bins` | Auth | Get supported card BINs for authorization mode; merchant maintains card packages |
...
| Get Card Privacy Info | GET | `/open-api/v1/card/{card_no}/private/info` | Common | Get card's CVV, expiry, card number (requires PCI permission) |
...
| Update Card Status | POST | `/open-api/v1/card/status/update` | Common | Update card usage status (Frozen-freeze / Activated-enable / Delete-delete) |
| Set PIN | POST | `/open-api/v1/card/pin/set` | Common | Set physical card PIN (requires encrypted PIN) |
| Card Status Change Notification | — | Webhook | Common | Notification when card status changes; also notified for risk control changes |
...
| **Cardholder Fund Transfer** | POST | `/open-api/v1/account/transfer` | Perform fund transfer when cardholder funds change (top-up or withdrawal); fixed currency TUSDT |
| Get Cardholder Account Info List | GET | `/open-api/v1/account/{cardholder_no}/info` | Get available assets in cardholder's credit account |
| Get Account Transfer Flow List | POST | `/open-api/v1/account/transfer/list` | Get cardholder's transfer history |
...
| --- | --- | --- |
| In | Company → Cardholder | User top-up (merchant account can be negative) |
| Out | Cardholder → Company | User withdrawal (requires sufficient balance) |
...
| Refund | Refund |
| Reversal | Chargeback/Cancellation |
...
[Webhook Auth Notification] Platform sends authorization notification
    ↓
Merchant must confirm within 1 second
    ↓
...
**Important**: Authorization mode has high performance requirements. Ensure your system can complete authorization response within 1 second.
...
**Q4: Why must authorization mode confirm within 1 second?**  
A: Authorization mode settles funds in real-time. Merchants must confirm within 1 second whether to allow the transaction to ensure smooth trading.

**Q5: How are consumption fees calculated?**  
A: Fee details are in the card package configuration at application time, including consumption rate, fees, exchange rate, etc. See "Constants" section for details.
