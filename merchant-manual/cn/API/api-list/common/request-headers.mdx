...
| Client-Access-Token | PCI iframe 内部接口使用 | 否 | 仅 PCI iframe 内部接口使用，**替代** `Access-Token` |
| Signature | 签名值 | 否 | **平台→商户回调**时使用，见第 10 章 |
...
- **获取 Token**（2.1）：仅需 `Api-Key` + `Timestamp`，无需 `Access-Token`
- **其他接口**：均需 `Api-Key` + `Timestamp` + `Access-Token`
- **PCI iframe 内部接口**（5.7.2）：使用 `Client-Access-Token`，**替代** `Access-Token`
...
- **沙箱环境**：`https://sandbox-openplatform.keysecure.io`
