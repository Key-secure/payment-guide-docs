...
| 账户 | POST | /open-api/v1/reserve/fund/transaction/list | 备付金余额明细列表 |
...
沙盒联调用接口：模拟卡片交易并向商户回调地址推送 Webhook。**不使用** 请求头中的 `Api-Key` / `Access-Token`。详见 [交易模拟说明](/cn/API/api-list/mock/overview)。
...
| 模拟 | POST | /refund | 模拟退款 |
| 模拟 | POST | /external_api/simulate_tevau_kyc | 模拟 KYC 审核（仅支持香港卡 BIN） |
...
**余额流程：**
1. 查询备付金余额明细 → `POST /reserve/fund/transaction/list`

**交易模拟流程：**
1. 模拟授权 → `POST /simulate/auth`
2. 模拟清算成功 / 失败 → `POST /simulate/consumption_clear` 或 `POST /simulate/consumption_fail`
3. 模拟冲正 / 退款 → `POST /simulate/reversal` 或 `POST /simulate/refund`
...
- `/reserve/fund/transaction/list` —— 余额明细
